...
**Phase 1 — Discovery:** For each package, the function queries Chainguard's PEP 503/691 Simple API indexes to find available distribution files (wheels and sdists). It compares these against what already exists in CodeArtifact and builds a list of work items (files to download and publish).
...
|--------------------|--------------------------|---------------------------------------------------------------|
| Sync lockfile       | `s3_bucket` + `s3_key`   | Sync exact versions from a `requirements.txt` stored in S3    |
| Inline requirements | `requirements_content`   | Sync versions from requirements content passed directly       |
...
This creates the CodeArtifact domain, upstream (public PyPI) repo, and main repo, then syncs all pinned packages from the requirements file.
...
If `SNS_TOPIC_ARN` is set, the function publishes a JSON summary to SNS on completion or failure. The subject line includes sync/failure counts and the run ID.
...
- Use `WHEEL_PYTHON_TAGS` and `WHEEL_PLATFORM_TAGS` to reduce the number of files per package.
