...
By default, syncs ALL available versions of each package listed in a
requirements.txt (not just the pinned version). Set SYNC_ALL_VERSIONS=false
or pass "sync_all_versions": false to revert to pinned-only behaviour.

Supports five Chainguard registry families:
...
Options (all optional):
    ...
    "sync_all_versions":   false  — only sync pinned versions (default: true)
...
Environment variables:
    ...
    # Feature flags
    SYNC_ALL_VERSIONS       — "true"/"false" — sync all Chainguard versions per package
                              (default: true)
    DRY_RUN                 — "true"/"false" — environment-level dry-run (default: false)
...
# When True (default), syncing a requirements.txt also pulls ALL versions of
# each listed package from Chainguard — not just the pinned version.  Set to
# "false" to only sync the exact pinned versions.
SYNC_ALL_VERSIONS = os.environ.get("SYNC_ALL_VERSIONS", "true").lower() != "false"

# Environment-level dry-run flag.  Can also be set per-invocation via event.
...
def _save_checkpoint(run_id, event, phase, work_items, counters,
    ...
    checkpoint = {
        ...
        "original_event": {
            ...
            "sync_all_versions": event.get("sync_all_versions", SYNC_ALL_VERSIONS),
...
def lambda_handler(event, context):
    ...
    Options:
       ...
       "dry_run":             true/false  — no writes                (default: env DRY_RUN or false)
       "sync_all_versions":   true/false  — sync all Chainguard      (default: env SYNC_ALL_VERSIONS or true)
    ...
    if sync_all_versions and not is_update_existing:
        logger.info("SYNC_ALL_VERSIONS enabled — all Chainguard versions will be synced "
                     "for each package in the requirements file")
    ...
    if phase == "discovery":
        ...
        _sync_all = sync_all_versions or original_event.get("sync_all_versions", False)
        logger.info("Discovery phase — starting at %d/%d (sync_all_versions=%s)",
                    discovery_index, len(package_names), _sync_all)
        ...
        try:
            for i in range(discovery_index, len(package_names)):
                ...
                if is_update or _sync_all:
                    # Update mode OR sync_all_versions: find ALL CGR versions
                    # not yet in CodeArtifact from standard registries.
                    found_in_registry = False
                    for reg_key, reg_url in _STANDARD_REGISTRIES:
                    ...
                    # For lockfile + sync_all_versions: also handle pinned CUDA
                    # versions which are NOT in standard registries.
                    if _sync_all and not is_update:
                        pinned = lockfile_versions.get(name, set())
                        synced_versions = {
                            w["version"] for w in work_items if w["name"] == name
                        } | existing
                        for version in sorted(pinned):
                            if version in synced_versions:
                                continue
                            registries = _determine_registries(version)
                            # Skip if this version would use standard registries
                            # (already covered by the full index scan above)
                            if registries == list(_STANDARD_REGISTRIES):
                                if not found_in_registry:
                                    not_found_list.append(f"{name}=={version}")
                                continue
                            # CUDA version — try its dedicated registry
                            found = False
                            for reg_key, reg_url in registries:
                                ...
                                if items:
                                    work_items.extend(items)
                                    logger.info("[%d/%d] %s==%s — %d file(s) from %s (CUDA)",
                    ...
                    if not is_update:
                        new_count = sum(1 for w in work_items if w["name"] == name)
                        if new_count:
                            logger.info("[%d/%d] %s — %d file(s) queued (all versions)",
                                        i + 1, len(package_names), name, new_count)
                        elif not found_in_registry:
                            logger.info("[%d/%d] %s — not found in any Chainguard registry",
                                        i + 1, len(package_names), name)
                else:
                    # Lockfile mode (sync_all_versions=false): only the pinned versions
                    pinned = lockfile_versions.get(name, set())
                    for version in sorted(pinned):
                        if version in existing:
                            counters["skipped_existing"] += 1
